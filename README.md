...
### How to use it
To generate gradient noise, first create a `gradient_noise` object then use the `operator()` overload to generate noise values. Noise values are between -1 and 1.
`gradient_noise` is declared inside the `gnd` namespace, can optionally be initialized with a seed, takes two template parameters. Construction might look like this: `gnd::gradient_noise<my_float_type, my_dimension_count> my_gradient_noise(my_seed)`. The first template parameter, `float_type`, determines the return type and input parameter type. The second template parameter, `dimension_count`, determines the number of dimensions. Optionally, `gradient_noise` can be constructed with a seed. If no seed is given, a default seed is used. Additionally, `gradient_noise` can be reseeded using the `seed` method. *Example code below.*
```
